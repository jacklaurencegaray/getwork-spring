...
public interface JobRequestRepository extends JpaRepository<JobRequest, Integer> {
    ...
    @Query("SELECT j FROM JobRequest j LEFT JOIN j.company c WHERE c.id = (:comp_id) AND j.status = (:findkey) OR j.jobRequestNumber like %:findkey%")
    List<JobRequest> findJobRequestsByJobRequestNumberOrStatus(@Param("findkey") String findkey, @Param("comp_id") Integer company_id);
}
